...
public class MySQLEbMSDAO extends AbstractEbMSDAO
{
	...
	public EbMSAttachment findAttachment(String messageId, int messageNr, String contentId)
	{
		...
		return jdbcTemplate.queryForObject("select a.name, a.content_id, a.content_type, a.content" + " from ebms_message m, ebms_attachment a"
				+ " where m.message_id = ?"
				+ " and m.message_nr = ?"
				+ " and m.id = a.ebms_message_id"
				+ " and a.content_id = ?",(rs, rowNum) ->
				{
					try
					{
						return new EbMSAttachment(rs.getString("name"),
				...
				},messageId,messageNr,contentId);
	}

	protected List<EbMSAttachment> getAttachments(String messageId, int messageNr)
	{
		return jdbcTemplate.query(
				"select a.name, a.content_id, a.content_type" + " from ebms_message m, ebms_attachment a"
						+ " where m.message_id = ?"
						+ " and m.message_nr = ?"
						+ " and m.id = a.ebms_message_id",
	...
	{
		jdbcTemplate.query("select a.name, a.content_id, a.content_type, a.content" + " from ebms_message m, ebms_attachment a"
				+ " where m.message_id = ?"
				+ " and m.message_nr = ?"
				+ " and m.id = a.ebms_message_id",(rs, rowNum) ->
				{
					try
					{
