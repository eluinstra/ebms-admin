...
public class MessageEditPage extends BasePage
{
	...
	public MessageEditPage(WebPage responsePage)
	{
		this(new EbMSMessage(),true,responsePage);
	}
	
	public MessageEditPage(final EbMSMessage message, WebPage responsePage)
	{
		this(message,false,responsePage);
	}	
	
	protected MessageEditPage(final EbMSMessage message, boolean isNew, WebPage responsePage)
	{
		add(new FeedbackPanel("feedback"));
		add(new EditMessageForm("editMessageForm",message,isNew,responsePage));
	}
	
	@Override
	public String getPageTitle()
	{
		return getLocalizer().getString("messageEdit",this);
	}

	public class EditMessageForm extends Form<EbMSMessage>
	{
		private static final long serialVersionUID = 1L;

		public EditMessageForm(String id, EbMSMessage message, final boolean isNew, final WebPage responsePage)
		{
			super(id,new CompoundPropertyModel<EbMSMessage>(message));
			...
			Button save = new Button("save",new ResourceModel("cmd.save"))
			{
				...
				public void onSubmit()
				{
					try
					{
						EbMSMessage cpa = (EbMSMessage)getParent().getDefaultModelObject();
						if (isNew)
							ebMSDAO.insert(cpa);
						else
							ebMSDAO.update(cpa);
						setResponsePage(new MessagesPage());
					}
					catch (Exception e)
					{
						logger.error("",e);
